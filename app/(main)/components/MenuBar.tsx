...
export default function MenuBar() {
    ...
    const [currentClickedMenu, setCurrentClickedMenu] = useState("home");
    const onClick = (e: React.MouseEvent<HTMLButtonElement>) => {
        const tagName = e.currentTarget.dataset.id as string;
        setCurrentClickedMenu(tagName);

        if (tagName === "search" || tagName === "notification") {
            setShowSlide(!showSlide);
        }
        else if (tagName === "create") {
    ...
    return (
        <div className={`hidden basis-auto md:flex md:flex-col overflow-hidden p-8 border-r border-r-slate-400 transition-width ${showSlide ? "w-[88px]" : "md:w-[88px] lg:w-[244px]"}`}>
            <div className="h-14 relative">
                <span className={`transition-opacity absolute top-0 left-0 ${showSlide ? "lg:opacity-0" : "lg:opacity-100 w-auto h-auto"}`}>
                ...
                <span className={`lg:opacity-0 transition-opacity absolute top-0 left-0 ${showSlide ? "lg:opacity-100" : "lg:opacity-0"}`}>
            ...
            <ul className="flex-grow py-4 flex flex-col gap-8">
                <li>
                    <Link href="/" >
                        <button data-id="home" className="flex gap-3" onClick={onClick}>
                            <span>
                                {
                                    currentClickedMenu === "home"
                            ...
                            <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "home" ? "font-bold" : "font-normal"} md:hidden lg:block transition-all`}>홈</span>
                        </button>
                    </Link>
                </li>
                <li>
                    <button data-id="search" className="flex gap-3" onClick={onClick}>
                        <span>
                            {currentClickedMenu === "search"
                        ...
                        <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "search" ? "font-bold" : "font-normal"} md:hidden lg:block transition-all`}>검색</span>
                    </button>
                </li>
                <li>
                    <Link href="/explore">
                        <button data-id="explore" className="flex gap-3" onClick={onClick}>
                            <span>
                                {currentClickedMenu === "explore"
                            ...
                            <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "explore" ? "font-bold" : "font-normal"} md:hidden lg:block transition-all`}>탐색 탭</span>
                        </button>
                    </Link>
                ...
                <li>
                    <Link href="/reels">
                        <button data-id="reels" className="flex gap-3" onClick={onClick}>
                            <span>
                                {currentClickedMenu === "reels"
                            ...
                            <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "reels" ? "font-bold" : "font-normal"} md:hidden lg:block transition-all`}>릴스</span>
                        </button>
                    </Link>
                </li>
                <li>
                    <Link href="/direct">
                        <button data-id="direct" className="flex gap-3" onClick={onClick}>
                            <span>
                                {currentClickedMenu === "direct"
                            ...
                            <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "direct" ? "font-bold" : "font-normal"} md:hidden lg:block transition-transform`}>메시지</span>
                        </button>
                    </Link>

                </li>
                <li>
                    <button data-id="notification" className="flex gap-3" onClick={onClick}>
                        <span>
                            {currentClickedMenu === "notification"
                        ...
                        <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "notification" ? "font-bold" : "font-normal"} md:hidden lg:block transition-transform`}>알림</span>
                    </button>
                </li>
                <li>
                    <button data-id="create" className="flex gap-3" onClick={onClick}>
                        <span>
                        ...
                        <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "create" ? "font-bold" : "font-normal"} md:hidden lg:block transition-transform`}>만들기</span>
                    </button>
                </li>
                <li>
                    <Link href="/profile" className="flex" >
                        <button data-id="profile" className="flex gap-3" onClick={onClick}>
                            <span>
                                <Image src="/images/profileImage.jpg" alt="profile image" width={24} height={24} />
                            </span>
                            <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "profile" ? "font-bold" : "font-normal"} md:hidden lg:block transition-transform`}>프로필</span>
                        </button>
                    </Link>
                </li>
            </ul>
            <div>
                <button data-id="setting" className="flex gap-3 relative" onClick={onClick}>
                    <span>
                        {currentClickedMenu === "setting"
                            ? <svg aria-label="설정" height="24" role="img" viewBox="0 0 24 24" width="24"><title>설정</title><path d="M3.5 6.5h17a1.5 1.5 0 0 0 0-3h-17a1.5 1.5 0 0 0 0 3Zm17 4h-17a1.5 1.5 0 0 0 0 3h17a1.5 1.5 0 0 0 0-3Zm0 7h-17a1.5 1.5 0 0 0 0 3h17a1.5 1.5 0 0 0 0-3Z"></path></svg>
                            : < svg aria-label="설정" fill="currentColor" height="24" role="img" viewBox="0 0 24 24" width="24"><title>설정</title><line fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" x1="3" x2="21" y1="4" y2="4"></line><line fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" x1="3" x2="21" y1="12" y2="12"></line><line fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" x1="3" x2="21" y1="20" y2="20"></line></svg>
                        }
                    </span>
                    <span className={`${showSlide ? "fixed left-full ml-2 scale-0" : "scale-100"} ${currentClickedMenu === "setting" ? "font-bold" : "font-normal"} md:hidden lg:block transition-transform`}>
                        더 보기
                    </span>
                </button>
            </div>
